...
Your job is to keep the research tightly grounded in the user's topic.

Rules:
- Prefer primary evidence queries: app store reviews, user forums, Telegram/VK/Reddit threads, official product posts, changelogs, tech media discussing the exact product.
- Use the exact product/company names from the query.
- Generate queries that help verify concrete facts, complaints, dates, platform-specific issues, and official responses.
- Avoid broad industry analogy or competitor-comparison queries unless the user explicitly asked for comparison.
- Avoid speculative or generic "strategy" queries when direct evidence queries are possible.
- Return queries that are diverse but still tightly focused on the same topic."""},
            {"role": "user",
             "content": f"""Given the following prompt, generate {num_queries} unique search queries to research the topic thoroughly.

Requirements:
- At least half of the queries should target direct evidence from user reactions or official product communication.
- Keep the queries fact-finding oriented, not generic thought pieces.
- If the prompt names platforms, communities, or publications, use them directly.
- Only include competitor comparisons if they are necessary to answer the prompt.
...
Format each question on a new line starting with 'Question: '"""}
    ...
    async def deep_research(
        ...
        async def process_query(serp_query: Dict[str, str]) -> Optional[Dict[str, Any]]:
            async with semaphore:
                try:
                    ...
                    # Share runtime budget/telemetry across nested researchers so
                    # soft limits apply to the whole deep-research tree, not each branch separately.
                    researcher.runtime_telemetry = self.researcher.runtime_telemetry
                    researcher.soft_limit_tavily_calls = self.researcher.soft_limit_tavily_calls
                    researcher.soft_limit_bedrock_tokens = self.researcher.soft_limit_bedrock_tokens
                    researcher.soft_limit_elapsed_seconds = self.researcher.soft_limit_elapsed_seconds
                    researcher.run_started_at_epoch = self.researcher.run_started_at_epoch
                    if self.researcher.runtime_telemetry.get("soft_limit", {}).get("triggered", False):
                        researcher.cfg.max_search_results_per_query = min(
                            int(researcher.cfg.max_search_results_per_query),
                            3,
                        )
        ...
        # Collect all results from the current level first.
        ...
        soft_triggered = bool(
            self.researcher.runtime_telemetry.get("soft_limit", {}).get("triggered", False)
        ) if hasattr(self.researcher, "runtime_telemetry") else False

        if depth > 1 and soft_triggered:
            logger.warning("Soft limit triggered: skipping deeper deep-research recursion")
        elif depth > 1:
