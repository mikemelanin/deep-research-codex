...
class ResearchConductor:
    ...
    async def conduct_research(self):
        ...
        # Hybrid search including both local documents and web sources
            ...
            if self.researcher.document_urls:
                from ..document import OnlineDocumentLoader
            ...
            from ..document import LangChainDocumentLoader
    ...
    async def _get_context_by_vectorstore(self, query, filter: dict | None = None):
        ...
        # Soft-limit guardrail: keep report generation alive while reducing spend.
        soft_cfg = self.researcher.runtime_telemetry.get("soft_limit", {})
        if hasattr(self.researcher, "_check_soft_limit"):
            self.researcher._check_soft_limit()
        max_subqueries = int(soft_cfg.get("max_subqueries", 0) or 0)
        if max_subqueries > 0 and len(sub_queries) > max_subqueries:
            if hasattr(self.researcher, "_trigger_soft_limit"):
                self.researcher._trigger_soft_limit(f"subqueries>={max_subqueries}")
            sub_queries = sub_queries[:max_subqueries]
            self.logger.warning(f"Soft limit: capped sub-queries to {max_subqueries}")
            if self.researcher.verbose:
                await stream_output(
                    "logs",
                    "soft_limit_subquery_cap",
                    f"⚠️ Soft limit active: limiting sub-queries to {max_subqueries}",
                    self.researcher.websocket,
                )
        ...
        soft_enabled = bool(self.researcher.runtime_telemetry.get("soft_limit", {}).get("enabled", False))
        if soft_enabled:
            context = []
            for sub_query in sub_queries:
                ...
                if self.researcher.runtime_telemetry.get("soft_limit", {}).get("triggered", False):
                    self.logger.warning("Soft limit triggered: stopping additional vectorstore sub-queries")
                    break
                context.append(await self._process_sub_query_with_vectorstore(sub_query, filter))
            return context
            ...
            if soft_enabled:
                ...
                for sub_query in sub_queries:
                    if hasattr(self.researcher, "_check_soft_limit"):
                        self.researcher._check_soft_limit()
                    if self.researcher.runtime_telemetry.get("soft_limit", {}).get("triggered", False):
                        self.logger.warning("Soft limit triggered: stopping additional web sub-queries")
                        break
                    context.append(await self._process_sub_query(sub_query, scraped_data, query_domains))
            else:
                context = await asyncio.gather(
                    *[
    ...
    async def _search_relevant_source_urls(self, query, query_domains: list | None = None):
        ...
        for retriever_class in self.researcher.retrievers:
            if hasattr(self.researcher, "_check_soft_limit"):
                self.researcher._check_soft_limit()
            if self.researcher.runtime_telemetry.get("soft_limit", {}).get("triggered", False):
                self.logger.warning("Soft limit triggered: stopping additional retriever searches")
                break
            ...
            try:
                ...
                is_tavily = retriever_class.__name__.lower() == "tavilysearch"
                if is_tavily:
                ...
                # Always queue discovered URLs for scraping to force full web crawl behavior.
                # Keep retriever-provided content only as fallback when a URL is missing.
                for result in search_results:
                    ...
                    if url:
                        ...
                        # Fallback when retriever returned text but no URL.
    ...
    async def _search(self, retriever, query):
        ...
        if hasattr(self.researcher, "_check_soft_limit"):
            self.researcher._check_soft_limit()
        if self.researcher.runtime_telemetry.get("soft_limit", {}).get("triggered", False):
            self.logger.warning(f"Soft limit already triggered, skipping search with {retriever_name} for query: {query}")
            return []
        
        try:
            # Instantiate the retriever
            is_tavily_retriever = retriever_name.lower() == "tavilysearch"
            retriever_instance = retriever(
                query=query, 
                headers=self.researcher.headers,
                query_domains=self.researcher.query_domains,
                websocket=self.researcher.websocket if is_mcp_retriever else None,
                researcher=self.researcher if (is_mcp_retriever or is_tavily_retriever) else None
            )
