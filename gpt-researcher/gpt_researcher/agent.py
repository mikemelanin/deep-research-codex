...
class GPTResearcher:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.runtime_telemetry: dict[str, Any] = {
            ...
            "soft_limit": {
                "enabled": str(os.getenv("SOFT_LIMIT_ENABLED", "1")).lower() not in ("0", "false", "no"),
                "triggered": False,
                "reason": "",
                "max_subqueries": int(os.getenv("SOFT_LIMIT_MAX_SUBQUERIES", "12")),
            },
        }
        self.soft_limit_tavily_calls = int(os.getenv("SOFT_LIMIT_TAVILY_CALLS", "25"))
        self.soft_limit_bedrock_tokens = int(os.getenv("SOFT_LIMIT_BEDROCK_TOTAL_TOKENS", "300000"))
        self.soft_limit_elapsed_seconds = int(os.getenv("SOFT_LIMIT_ELAPSED_SECONDS", "600"))
        self.run_started_at_epoch = int(os.getenv("RUN_STARTED_AT_EPOCH", "0") or 0)
    ...
    def _emit_telemetry(self, event: dict[str, Any]) -> None:
        try:
            print(f"TELEMETRY_JSON {json.dumps(event, ensure_ascii=False)}", flush=True)
        except Exception:
            return

    def _trigger_soft_limit(self, reason: str) -> None:
        soft = self.runtime_telemetry["soft_limit"]
        if not soft.get("enabled") or soft.get("triggered"):
            return
        soft["triggered"] = True
        soft["reason"] = reason
        # Keep report generation alive while reducing spend.
        self.cfg.max_search_results_per_query = min(int(self.cfg.max_search_results_per_query), 3)
        self._emit_telemetry({
            "type": "soft_limit_triggered",
            "reason": reason,
            "max_search_results_per_query": self.cfg.max_search_results_per_query,
            "max_subqueries": soft.get("max_subqueries", 12),
        })

    def _check_soft_limit(self) -> None:
        soft = self.runtime_telemetry["soft_limit"]
        if not soft.get("enabled") or soft.get("triggered"):
            return
        reasons: list[str] = []
        bedrock = self.runtime_telemetry["bedrock"]
        tavily = self.runtime_telemetry["tavily"]
        total_bedrock_tokens = (
            int(bedrock.get("input_tokens", 0))
            + int(bedrock.get("output_tokens", 0))
            + int(bedrock.get("cache_read_input_tokens", 0))
            + int(bedrock.get("cache_write_input_tokens", 0))
        )
        if self.soft_limit_tavily_calls > 0 and int(tavily.get("calls", 0)) >= self.soft_limit_tavily_calls:
            reasons.append(f"tavily_calls>={self.soft_limit_tavily_calls}")
        if self.soft_limit_bedrock_tokens > 0 and total_bedrock_tokens >= self.soft_limit_bedrock_tokens:
            reasons.append(f"bedrock_tokens>={self.soft_limit_bedrock_tokens}")
        if self.soft_limit_elapsed_seconds > 0 and self.run_started_at_epoch > 0:
            import time
            elapsed = int(time.time()) - self.run_started_at_epoch
            if elapsed >= self.soft_limit_elapsed_seconds:
                reasons.append(f"elapsed>={self.soft_limit_elapsed_seconds}s")
        if reasons:
            self._trigger_soft_limit("; ".join(reasons))

    def record_tavily_usage(self, usage: dict | None = None, success: bool = True) -> None:
        tavily = self.runtime_telemetry["tavily"]
        tavily["calls"] = int(tavily.get("calls", 0)) + 1
        if not success:
            tavily["failed_calls"] = int(tavily.get("failed_calls", 0)) + 1

        usage = usage or {}
        request_credits = 0.0
        for key, value in usage.items():
            if isinstance(value, (int, float)):
                tavily["raw_usage"][key] = tavily["raw_usage"].get(key, 0) + value
        for credit_key in ("credits_used", "request_credits", "total_credits", "credits"):
            val = usage.get(credit_key)
            if isinstance(val, (int, float)):
                request_credits = float(val)
                break
        if request_credits > 0:
            tavily["estimated_credits"] = float(tavily.get("estimated_credits", 0.0)) + request_credits
        ...
        self._emit_telemetry({
            ...
            "totals": {
                "calls": tavily["calls"],
                "failed_calls": tavily["failed_calls"],
                "estimated_credits": tavily["estimated_credits"],
            },
        })
        self._check_soft_limit()

    def add_costs(self, cost: float | dict[str, Any]) -> None:
        ...
        if isinstance(cost, dict):
            ...
            bedrock = self.runtime_telemetry["bedrock"]
            bedrock["calls"] = int(bedrock.get("calls", 0)) + 1
            bedrock["input_tokens"] = int(bedrock.get("input_tokens", 0)) + input_tokens
            bedrock["output_tokens"] = int(bedrock.get("output_tokens", 0)) + output_tokens
            bedrock["cache_read_input_tokens"] = int(bedrock.get("cache_read_input_tokens", 0)) + cache_read_input_tokens
            bedrock["cache_write_input_tokens"] = int(bedrock.get("cache_write_input_tokens", 0)) + cache_write_input_tokens
            step = self._current_step
            step_usage = bedrock["by_step"].setdefault(step, {
                "calls": 0,
                "input_tokens": 0,
                "output_tokens": 0,
                "cache_read_input_tokens": 0,
            ...
            step_usage["calls"] += 1
            step_usage["input_tokens"] += input_tokens
            step_usage["output_tokens"] += output_tokens
            step_usage["cache_read_input_tokens"] += cache_read_input_tokens
            step_usage["cache_write_input_tokens"] += cache_write_input_tokens
            amount = float(cost.get("estimated_cost_usd", 0.0) or 0.0)
            bedrock["estimated_cost_usd"] = float(bedrock.get("estimated_cost_usd", 0.0)) + amount
            step_usage["estimated_cost_usd"] += amount
            self._emit_telemetry({
                "type": "bedrock_usage_step",
                "step": step,
            ...
            self.step_costs[step] = self.step_costs.get(step, 0.0) + amount
            self._check_soft_limit()
            return

        if not isinstance(cost, (float, int)):
            raise ValueError("Cost must be an integer, float, or telemetry payload dict")
