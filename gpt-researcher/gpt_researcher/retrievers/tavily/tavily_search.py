...
class TavilySearch:
    ...
    def __init__(self, query, headers=None, topic="general", query_domains=None, researcher=None, **kwargs):
        """
    ...
    def _search(
        ...
        if response.status_code == 200:
            ...
            if self.researcher and hasattr(self.researcher, "record_tavily_usage"):
                self.researcher.record_tavily_usage(usage=usage, success=True)
            try:
                print(
                    f"TELEMETRY_JSON {json.dumps({'type': 'tavily_usage', 'usage': usage}, ensure_ascii=False)}",
                    flush=True,
                )
            except Exception:
                pass
            ...
            if self.researcher and hasattr(self.researcher, "record_tavily_usage"):
                self.researcher.record_tavily_usage(usage={}, success=False)
