...
from .costs import estimate_bedrock_cost_from_usage, estimate_llm_cost


def _emit_runtime_telemetry(event: dict[str, Any]) -> None:
    """Emit a parse-friendly telemetry line to stdout."""
    try:
        print(f"TELEMETRY_JSON {json.dumps(event, ensure_ascii=False)}", flush=True)
    except Exception:
        # Never break research flow due to telemetry formatting.
        return
...
async def create_chat_completion(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if cost_callback:
            ...
            model_name = getattr(provider, "last_model_name", "") or model or ""
            if llm_provider == "bedrock" and usage:
                llm_costs = estimate_bedrock_cost_from_usage(usage, model_name)
                payload = {
                    "type": "bedrock_usage",
                    "provider": llm_provider,
                    "model": model_name,
                    "usage": usage,
                    "estimated_cost_usd": llm_costs,
                }
                _emit_runtime_telemetry(payload)
                try:
