...
- You MUST prioritize the relevance, reliability, and significance of the sources you use. Choose trusted sources over less reliable ones.
- You must also prioritize new articles over older articles if the source can be trusted.
- Do NOT introduce competitor analogies, market-wide generalizations, or product-strategy speculation unless they are directly supported by the provided context.
- Separate verified findings from inference. If evidence is partial, say so explicitly instead of filling gaps with generic reasoning.
- Prefer concrete user complaints, exact platform behaviors, dates, release notes, official posts, and directly observed regressions over broad UX theory.
- If the available evidence is thin or contradictory, narrow the claim and state the limitation clearly.
- Do NOT invent references, metrics, ratings, churn, or official responses that are not present in the provided context.
- Use in-text citation references in {report_format} format and make it with markdown hyperlink placed at the end of the sentence or paragraph that references them like this: ([in-text citation](url)).
- {tone_prompt}
- Write in {language}
