...
class GenericLLMProvider:
    ...
    def from_provider(cls, provider: str, chat_log: str | None = None, verbose: bool=True, **kwargs: Any):
        if provider == "openai":
            ...
            if "model" in kwargs or "model_name" in kwargs:
                model_id = kwargs.pop("model", None) or kwargs.pop("model_name", None)
                # Keep Bedrock client-level params at top-level so timeouts/retries are applied.
                client_level_keys = {
                    "timeout",
                    "max_retries",
                    "config",
                ...
                client_kwargs = {
                    ...
                    if key in client_level_keys
                }
                kwargs = {"model_id": model_id, "model_kwargs": kwargs, **client_kwargs}
